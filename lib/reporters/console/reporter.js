  ...
  _.each(results.filesByName, function(fileByName, key) {
    if (fileByName.length > 1) {
      ...
      _.each(fileByName, function(file) {
        var str =  '    ' + file.path.blue + (' (md5: ' + file.md5 + ')').magenta;

        if (file.exifData) {
          str += (' [Made with: "' + file.exifData.image.Make + ' - ' + file.exifData.image.Model + '", date: ' + file.exifData.exif.DateTimeOriginal + ']').red;
        }
