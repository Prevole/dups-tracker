...
module.exports = function(results, options) {
  ...
  htmlContent.append('<table><thead><tr><th></th><th></th></tr></thead><tbody>');
  ...
  _.each(results.filesByName, function(fileByName, key) {
    if (fileByName.length > 1) {
      ...
      _.each(fileByName, function(file) {
        ...
        htmlContent.append('<span style="color: blue"><a target="_blank" href="file://' + path.resolve(file.path) + '">' + file.path + '</a></span> <span style="color: magenta;">(md5: ' + file.md5 + ')</span></td><td>');

        if (file.exifData) {
          htmlContent.append('<span style="color: red;">[Made with: "' + file.exifData.image.Make + ' - ' + file.exifData.image.Model + '", date: ' + file.exifData.exif.DateTimeOriginal + ']</span>');
        }
  ...
  _.each(results.filesByMd5, function(fileByMd5, key) {
    if (fileByMd5.length > 1) {
      ...
      _.each(fileByMd5, function(file) {
        htmlContent.append('<tr><td style="padding-left: 10px;">');
        htmlContent.append('<span style="color: blue"><a target="_blank" href="file://' + path.resolve(file.path) + '">' + file.path + '</a></span></td><td>');
